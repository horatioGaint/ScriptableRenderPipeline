...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    ...
    {
        enum KernelType { Tex2D, Tex2DArray, Count };
        ...
        {
            m_TempColorTargets = new RTHandle[(int)KernelType.Count];
        ...
        public void Release()
        {
            for (int i = 0; i < (int)KernelType.Count; ++i)
            {
        ...
        {
            int[] colorKernels = new int[(int)KernelType.Count];
            colorKernels[(int)KernelType.Tex2D] = m_ColorPyramidCS.FindKernel(name);
            colorKernels[(int)KernelType.Tex2DArray] = m_ColorPyramidCS.FindKernel(name + "_Tex2DArray");
        ...
        public void RenderMinDepthPyramid(CommandBuffer cmd, RenderTexture texture, HDUtils.PackedMipChainInfo info)
        {
            ...
            Debug.Assert(XRGraphics.computePassCount == texture.volumeDepth);
        ...
        public int RenderColorGaussianPyramid(CommandBuffer cmd, Vector2Int size, Texture source, RenderTexture destination)
        {
            ...
            int kernelIndex = (int)(source.dimension == TextureDimension.Tex2DArray ? KernelType.Tex2DArray : KernelType.Tex2D);
            ...
            #if UNITY_SWITCH
            bool preferFragment = true; // XRTODO: fix ps path
            #else
            ...
            Debug.Assert(XRGraphics.computePassCount == slices);
